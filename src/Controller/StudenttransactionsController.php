...
class StudenttransactionsController extends AppController
{
	...
	public function reporteGeneralMorosidadRepresentantes($mes_desde = null, $mes_hasta = null, $periodo_escolar = null, $tipo_reporte = null, $consejo_educativo, $indicador_recalculo = null, $telefono = null, $tipoEstudiante = 'Todos', $condicionRegulares = 'No', $condicionEgresados = 'No')
	{
		...
		$anio_correspondiente_mes = $anio + $mes_ubicacion_anio[$mes_desde];

		$anio_mes_dia_desde = $anio_correspondiente_mes.$mes_desde."01";

		$mes_anio_desde = $mes_desde."/".$anio_correspondiente_mes;

		// Hasta

		$anio_correspondiente_mes = $anio + $mes_ubicacion_anio[$mes_hasta];

		$anio_mes_dia_hasta = $anio_correspondiente_mes.$mes_hasta."01";

		$mes_anio_hasta = $mes_hasta."/".$anio_correspondiente_mes;
		...
		$vector_cuotas = $this->saldoCuotas($mes_desde, $mes_hasta, $anio, $periodo_escolar, $indicador_recalculo, $transacciones_estudiantes, $anio_mes_dia_desde, $anio_mes_dia_hasta, $anio_mes_dia_agosto);
	...
	public function saldoCuotas($mes_desde = null, $mes_hasta = null, $anio = null, $periodo_escolar = null, $indicador_recalculo = null, $transacciones = null, $anio_mes_dia_desde, $anio_mes_dia_hasta = null, $anio_mes_dia_agosto = null)
	{
		...
		foreach ($transacciones as $transaccion)
		{
			if (isset($vectorEstudiantesInscritos[$transaccion->student->id]))
			{
				...
				// En la variable $representante guardar el registro del representante del estudiante al cual pertenece la transacción
				$representante = $transaccion->student->parentsandguardian;
				...
				{
					$anio_transaccion = $transaccion->ano_escolar;
					$mes_transaccion = '00';
				}
                ...
                if ($anio_mes_dia_transaccion <= $anio_mes_dia_hasta || $anio_mes_dia_transaccion == $anio_mes_dia_agosto)
                {
                    ...
                    elseif ($transaccion->transaction_type == 'Mensualidad' && substr($transaccion->transaction_description, 0, 3) != 'Ago')
                    {
                        ...
                        elseif ($anio == $anio_escolar_actual)
                        {
	...
	public function vectorConsejoEducativo($anio = null, $anioEscolarActual = null, $anioInscripcionActual = null, $periodo_escolar = null, $total_cuotas_periodo = 0, $detalle_morosos = [], $vector_morosidad = null, $totales_morosidad = null, $grados = 'Todos', $funcionLlamadora = null, $condicionRegulares = 'No', $condicionEgresados = 'No', $soloMorosos = true)
	{
		...
		foreach ($matriculas_estudiantes as $matricula)
		{
			...
			{
                $resumen['contadorFamilias']++;

                $recibosConceptos = (clone $recibosConceptosQuery)->where(['Bills.parentsandguardian_id' => $representante->id]);

                $resultado = $parentsController->procesarConsejoEducativo($anioEscolarActual, $proximoAnioEscolar, $representante, $otrasTarifas, $anio, $recibosConceptos);
                ...
                if ($dataAnio)
                {
                    ...
                    if ($dataAnio['consejoExonerado'] > 0) {
                        ...
                        $esExoneradaORelacionada = true;
                    } elseif ($dataAnio['idFamiliaPagadoraConsejo'] > 0) {
                        $resumen['totalFamiliasRelacionadas']++;
                        $esExoneradaORelacionada = true;
                    }

                    if (!$esExoneradaORelacionada) {
                        $resumen['proyeccionCobroFamilias'] += $dataAnio['tarifa'];
                        if ($dataAnio['indicadorReciboConsejo']) {
                            $resumen['cobradoFamilias'] += $dataAnio['tarifa'];
                        } else {
                            $resumen['pendientePorPagoFamilias'] += $dataAnio['tarifa'];
                        }
                    } else {
                        if ($dataAnio['pagado'] > 0) {
                            $resumen['cobradoFamilias'] += $dataAnio['pagado'];
                        }
                    }

                    $infoFamilia = [
                        'exoneradaORelacionada' => $esExoneradaORelacionada,
	...
	*/
    ...
    public function consultaDeudaRepresentante($idUsuario = null, $rolUsuario = null, $idRepresentante = null, $idEstudiante = null, $controlador = 'Users', $accion = 'wait')
    {
		...
		if ($idRepresentante > 0)
		{
			...
			$totalDeudaEstudianteAnio = 0;
			$primerAnioInscripcionEstudiante = '';
			$ultimoAnioInscripcionEstudiante = '';
			$aniosEstudiante = [];
			...
			[
				'Studenttransactions.invoiced'  => 0,
				'Studenttransactions.ano_escolar >=' => $anioEscolarAnterior,
			...
			if ($contadorTransaccionesEncontradas > 0)
			{
				foreach ($transaccionesEstudiantes as $indice => $transaccion)
				{
					...
					if ($contadorTransacciones == 0)
					{
						...
						$primerAnioInscripcionEstudiante = $anioEscolarActual;
						$ultimoAnioInscripcionEstudiante = $anioEscolarActual;
						$aniosEstudiante[] = $anioEscolarActual;
					}
					$contadorTransacciones++;
					if ($idEstudianteAnterior != $idEstudianteActual)
					{
						$vectorAnios[$anioEscolarAnterior] =
							[
								'total_deuda_estudiante_anio' => $totalDeudaEstudianteAnio,
								'cuotas_anio_escolar' => $cuotasAnioEscolar
							];
						$vectorEstudiantes[$idEstudianteAnterior] =
							[
								'nombre_estudiante' => $nombreEstudianteAnterior,
								'total_deuda_estudiante' => $totalDeudaEstudiante,
								'primer_anio_inscripcion_estudiante' => $primerAnioInscripcionEstudiante,
								'ultimo_anio_inscripcion_estudiante' => $ultimoAnioInscripcionEstudiante,
								'anios_estudiante' => $aniosEstudiante,
								'anios' => $vectorAnios
							];
						$totalDeudaEstudiante = 0;
						$totalDeudaEstudianteAnio = 0;
					...
					$respuestaProcesarTransaccion = $this->procesarTransaccion($rolUsuario, $anioEscolarActual, $transaccion, $totalDeudaRepresentante, $totalDeudaEstudiante, $totalDeudaEstudianteAnio, $cuotasAnioEscolar, $representante);
					...
					$totalDeudaEstudianteAnio = $respuestaProcesarTransaccion['total_deuda_estudiante_anio'];
					$cuotasAnioEscolar = $respuestaProcesarTransaccion['cuotas_anio_escolar'];
				}
				$vectorAnios[$anioEscolarAnterior] =
					[
						'total_deuda_estudiante_anio' => $totalDeudaEstudianteAnio,
		...
		$this->set(compact('idUsuario', 'rolUsuario', 'idRepresentante', 'representante', 'contadorTransaccionesEncontradas', 'vectorEstudiantes', 'totalDeudaRepresentante', 'controlador', 'accion'));
    }

	public function procesarTransaccion($rolUsuario = null, $anioEscolarActual = null, $transaccion = null, $totalDeudaRepresentante = null, $totalDeudaEstudiante = null, $totalDeudaEstudianteAnio = null, $cuotasAnioEscolar = null, $representante = null)
	{
		$registrarCuota = 0;
		$pendienteCuota = 0;
		$pendienteCuotaActual = 0;
		$indicadorCuotaVencida = 1;

		$fechaHoraActual = Time::now();
		$anioActual = $fechaHoraActual->year;
		if ($fechaHoraActual->month < 10)
		{
			$mesActual = '0'.$fechaHoraActual->month;
		}
		else
		{
			$mesActual = $fechaHoraActual->month;
		}
		$anioMesActual = $anioActual.$mesActual;
		...
		if ($registrarCuota == 1)
		{
			if ($transaccion->transaction_type == 'Matrícula' || substr($transaccion->transaction_description, 0, 3) == 'Ago')
			{
				...
				$tarifaCuota = $respuestaProcesarMatriculaAgosto['tarifaConDescuentoRecargo'];
				$porcentajeDescuento = $respuestaProcesarMatriculaAgosto['porcentajeDescuento'];
				$montoAbono = $respuestaProcesarMatriculaAgosto['montoAbono'];
				$pendienteCuota = $respuestaProcesarMatriculaAgosto['saldoCuota'];
				if ($pendienteCuota < 0)
				{
					$pendienteCuota = 0;
				}
				$pendienteCuotaActual = $pendienteCuota;

				$tarifaAnticipoAbono = $respuestaProcesarMatriculaAgosto['tarifaAnticipoAbono'];
				$montoAbonoAnticipoAbono = $respuestaProcesarMatriculaAgosto['montoAbonoAnticipoAbono'];
				$saldoAnticipoAbonoInscripcion = $respuestaProcesarMatriculaAgosto['saldoAnticipoAbonoInscripcion'];
				if ($saldoAnticipoAbonoInscripcion < 0)
				{
					$saldoAnticipoAbonoInscripcion = 0;
				}

				$tarifaDiferencia = $respuestaProcesarMatriculaAgosto['tarifaDiferencia'];
				$montoAbonoDiferencia = $respuestaProcesarMatriculaAgosto['montoAbonoDiferencia'];
				$saldoDiferenciaInscripcion = $respuestaProcesarMatriculaAgosto['saldoDiferenciaInscripcion'];
				if ($saldoDiferenciaInscripcion < 0)
				{
					$saldoDiferenciaInscripcion = 0;
				}

				$indicadorCuotaVencida = 1;

				$proximoAnioEscolarTransaccion = $transaccion->ano_escolar + 1;
				...
				if ($rolUsuario == 'Representante')
				{
					if ($transaccion->transaction_type == 'Matrícula')
					{
						...
						[
							'id' => $transaccion->id,
							'tipoCuota' => 'Anticipo matrícula',
							'cuota' => 'Anticipo matrícula '.$transaccion->ano_escolar,
							'tarifaCuota' => $tarifaAnticipoAbono,
							'porcentajeDescuento' => $porcentajeDescuento,
							'montoAbono' => $montoAbonoAnticipoAbono,
							'pendienteCuota' => $saldoAnticipoAbonoInscripcion,
							'indicadorCuotaVencida' => $indicadorCuotaVencida
						];
						if ($indicadorExoneradoDiferenciasInscripcion == 1)
						{
							$cuotasAnioEscolar[] =
							[
								'id' => $transaccion->id,
								'tipoCuota' => 'Diferencia matrícula',
								'cuota' => 'Diferencia matrícula '.$transaccion->ano_escolar,
								'tarifaCuota' => $tarifaDiferencia,
								'porcentajeDescuento' => $porcentajeDescuento,
								'montoAbono' => 0,
								'pendienteCuota' => 0,
								'indicadorCuotaVencida' => $indicadorCuotaVencida
						...
						else
						{
							...
							[
								...
								'tarifaCuota' => $tarifaDiferencia,
								'porcentajeDescuento' => $porcentajeDescuento,
								'montoAbono' => $montoAbonoDiferencia,
								'pendienteCuota' => $saldoDiferenciaInscripcion,
								'indicadorCuotaVencida' => $indicadorCuotaVencida
							];
						}
					}
					else
					{
						...
						[
							'id' => $transaccion->id,
							'tipoCuota' => 'Abono agosto',
							'cuota' => 'Abono agosto '.$proximoAnioEscolarTransaccion,
							'tarifaCuota' => $tarifaAnticipoAbono,
							'porcentajeDescuento' => $porcentajeDescuento,
							'montoAbono' => $montoAbonoAnticipoAbono,
							'pendienteCuota' => $saldoAnticipoAbonoInscripcion,
						...
						if ($indicadorExoneradoDiferenciasInscripcion == 1)
						{
							...
							[
								'id' => $transaccion->id,
								'tipoCuota' => 'Diferencia agosto',
								'cuota' => 'Diferencia agosto '.$proximoAnioEscolarTransaccion,
								'tarifaCuota' => $tarifaDiferencia,
								'porcentajeDescuento' => $porcentajeDescuento,
								'montoAbono' => 0,
								'pendienteCuota' => 0,
				...
				else
				{
					...
					[
						'id' => $transaccion->id,
						'tipoCuota' => $transaccion->transaction_type,
			...
			elseif ($transaccion->transaction_type == 'Servicio educativo')
			{
				...
				$tarifaCuota = $respuestaProcesarServicioEducativo['tarifaCuota'];
				$porcentajeDescuento = $respuestaProcesarServicioEducativo['porcentajeDescuento'];
				$montoAbono = $respuestaProcesarServicioEducativo['montoAbono'];
				$pendienteCuota = $respuestaProcesarServicioEducativo['saldoCuota'];
				$pendienteCuotaActual = $pendienteCuota;
				$indicadorCuotaVencida = 1;
			...
			elseif ($transaccion->transaction_type == 'Mensualidad' && substr($transaccion->transaction_description, 0, 3) != 'Ago')
			{
				...
				$pendienteCuota = $respuestaProcesarMensualidad['saldoCuota'];
			...
			$totalDeudaRepresentante = round($totalDeudaRepresentante + $pendienteCuotaActual, 2);
			$totalDeudaEstudiante = round($totalDeudaEstudiante + $pendienteCuotaActual, 2);
			$totalDeudaEstudianteAnio = round($totalDeudaEstudianteAnio + $pendienteCuotaActual, 2);

		}
		$respuesta =
			[
				'total_deuda_representante' => $totalDeudaRepresentante,
				'total_deuda_estudiante' => $totalDeudaEstudiante,
				'total_deuda_estudiante_anio' => $totalDeudaEstudianteAnio,
				'cuotas_anio_escolar' => $cuotasAnioEscolar
			];
	...
	public function procesarMatriculaAgosto($transaccion = null, $tipoDocumentoIdentidad = null, $numeroDocumentoIdentidad = null)
	{
		$tarifasMatriculaAgosto =
			[
				'Matrícula 2023' =>  ['tarifa' => 190, 'anticipoAbono' => 120, 'diferencia' => 70, 'descuentoRecargo' => 0],
				'Ago 2024' => ['tarifa' => 190, 'anticipoAbono' => 120, 'diferencia' => 70, 'descuentoRecargo' => 0],

				'Matrícula 2024' =>  ['tarifa' => 220, 'anticipoAbono' => 190, 'diferencia' => 30, 'descuentoRecargo' => 0],
				'Ago 2025' => ['tarifa' => 220, 'anticipoAbono' => 190, 'diferencia' => 20, 'descuentoRecargo' => -10],

				'Matrícula 2025' =>  ['tarifa' => 263, 'anticipoAbono' => 220, 'diferencia' => 43, 'descuentoRecargo' => 0],
				'Ago 2026' => ['tarifa' => 263, 'anticipoAbono' => 220, 'diferencia' => 43, 'descuentoRecargo' => 0],

				'Matrícula 2026' =>  ['tarifa' => 263, 'anticipoAbono' => 263, 'diferencia' => 0, 'descuentoRecargo' => 0],
				'Ago 2027' => ['tarifa' => 263, 'anticipoAbono' => 263, 'diferencia' => 0, 'descuentoRecargo' => 0],
			];

		$codigo_retorno = 0;
		$mensaje = 'Proceso exitoso';
		$indicadorExoneradoDiferenciasInscripcion = 0;
		$indicadorDeudaInscripcion = 1;

		$tarifaCuota = $tarifasMatriculaAgosto[$transaccion->transaction_description]['tarifa'];
		$tarifaConDescuentoRecargo = round($tarifasMatriculaAgosto[$transaccion->transaction_description]['tarifa'] + $tarifasMatriculaAgosto
		[$transaccion->transaction_description]['descuentoRecargo'], 2);
		$porcentajeDescuento = 0;
		$montoAbono = 0;
		$saldoCuota = 0;

		$tarifaAnticipoAbono = $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'];
		$montoAbonoAnticipoAbono = 0;
		$saldoAnticipoAbonoInscripcion = $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'];

		$tarifaDiferencia = $tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'];
		$montoAbonoDiferencia = 0;
		$saldoDiferenciaInscripcion = $tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'];

		$respuesta = '';

		$vectorColaboradorsExoneradosDiferencias = [];
		$this->loadModel('Colaboradors');
		$colaboradors = $this->Colaboradors->find('all');
		foreach ($colaboradors as $indiceColaborador => $colaborador)
		{
			$vectorColaboradorsExoneradosDiferencias[$colaborador->tipo_documento_identidad.$colaborador->numero_documento_identidad] = $colaborador->tipo_documento_identidad.$colaborador->numero_documento_identidad;
		}

		if (isset($vectorColaboradorsExoneradosDiferencias[$tipoDocumentoIdentidad.$numeroDocumentoIdentidad]))
		{
			$indicadorExoneradoDiferenciasInscripcion = 1;
		}

		$montoAbono = $transaccion->amount_dollar;
		$montoPagadoNeto = $montoAbono;
		if ($transaccion->paid_out == 0)
		{
			$indicadorDeudaInscripcion = 1;
			$saldoCuota = round($tarifaConDescuentoRecargo - $transaccion->amount_dollar, 2);
            if ($indicadorExoneradoDiferenciasInscripcion == 1 || $transaccion->student->section->orden > 41)
			{
                $saldoCuota = round($saldoCuota - $tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'], 2);
            }

			if ($transaccion->amount_dollar < $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'])
			{
				$montoAbonoAnticipoAbono = $transaccion->amount_dollar;
				$saldoAnticipoAbonoInscripcion = round($tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'] - $montoAbonoAnticipoAbono, 2);
			...
			elseif ($transaccion->amount_dollar > $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'])
			{
				$montoAbonoAnticipoAbono = $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'];
				$saldoAnticipoAbonoInscripcion = 0;

				$montoAbonoDiferencia = round($transaccion->amount_dollar - $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'], 2);
				$saldoDiferenciaInscripcion = round($tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'] - $montoAbonoDiferencia, 2);
			}
		}
		else
		{
			if ($tarifaConDescuentoRecargo == $transaccion->amount_dollar)
			{
				$indicadorDeudaInscripcion = 0;
				$saldoCuota = 0;
				$montoAbonoAnticipoAbono = $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'];
				$saldoAnticipoAbonoInscripcion = 0;
				$montoAbonoDiferencia = $tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'];
				$saldoDiferenciaInscripcion = 0;
			}
			elseif ($tarifaConDescuentoRecargo > $transaccion->amount_dollar)
			{
				if ($indicadorExoneradoDiferenciasInscripcion == 1 || $transaccion->student->section->orden > 41)
				{
					$indicadorDeudaInscripcion = 0;
				...
				{
					$indicadorDeudaInscripcion = 1;
					$saldoCuota = $tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'];
					$montoAbonoAnticipoAbono = $tarifasMatriculaAgosto[$transaccion->transaction_description]['anticipoAbono'];
					$saldoAnticipoAbonoInscripcion = 0;
					$montoAbonoDiferencia = 0;
					$saldoDiferenciaInscripcion = $tarifasMatriculaAgosto[$transaccion->transaction_description]['diferencia'];
		...
		$respuesta =
			[
				...
				'indicadorExoneradoDiferenciasInscripcion' => $indicadorExoneradoDiferenciasInscripcion,
				'indicadorDeudaInscripcion' => $indicadorDeudaInscripcion,

				'tarifaCuota' => $tarifaCuota,
				'tarifaConDescuentoRecargo' => $tarifaConDescuentoRecargo,
				'porcentajeDescuento' => $porcentajeDescuento,
				'montoAbono' => $montoAbono,
				'montoPagadoNeto' => $montoPagadoNeto,
				'saldoCuota' => $saldoCuota,

				'tarifaAnticipoAbono' => $tarifaAnticipoAbono,
				'montoAbonoAnticipoAbono' => $montoAbonoAnticipoAbono,
				'saldoAnticipoAbonoInscripcion' => $saldoAnticipoAbonoInscripcion,

				'tarifaDiferencia' => $tarifaDiferencia,
				'montoAbonoDiferencia' => $montoAbonoDiferencia,
				'saldoDiferenciaInscripcion' => $saldoDiferenciaInscripcion,
			];
		return $respuesta;
	}
	public function procesarServicioEducativo($transaccion = null, $estudiante = null)
	{
		$codigo_retorno = 0;
		$mensaje = 'Proceso exitoso';
		...
		if ($estudiante == null)
		{
			$idEstudiante = $transaccion->student_id;
			$estudiante = $this->Studenttransactions->Students->get($idEstudiante, [
				'contain' => []
			]);
		}
		...
		if ($tarifaCuota > 0)
		{
			...
			elseif ($transaccion->paid_out == 1)
			{
				$tarifaCuota = $transaccion->amount;
				$saldoCuota = 0;
			}
			elseif ($transaccion->partial_payment == 1)
			{
				$tarifaCuota = $transaccion->amount;
			...
			if ($saldoCuota == 0)
			{
				$indicadorDeudaServicioEducativo = 0;
			}
		}
		else
		{
	...
	public function procesarMensualidad($transaccion = null, $indicadorRecalculoCuota = null, $soloCuotasVencidas = null)
	{
		...
		else
		{
			...
			{
				$porcentajeDescuento = $transaccion->student->descuento_ano_anterior;
			}
			elseif ($transaccion->ano_escolar == $anioEscolarActual)
			{
				$porcentajeDescuento = $transaccion->student->discount;
			}
		}
		...
		$tarifaConDescuentoRecargo = round($tarifaCuota * $porcentajeDescuentoDecimal, 2);

		$montoAbono = $transaccion->amount_dollar;
		$montoPagadoNeto = $transaccion->amount_dollar;
		if ($transaccion->paid_out == 1)
		{
			if ($tarifaCuota == $tarifaConDescuentoRecargo)
			{
		...
		$respuesta =
			[
				'codigo_retorno' => $codigo_retorno,
				'mensaje' => $mensaje,
				'indicadorDeudaMensualidad' => $indicadorDeudaMensualidad,
				'tarifaCuota' => $tarifaCuota,
				'tarifaConDescuentoRecargo' => $tarifaConDescuentoRecargo,
				'porcentajeDescuento' => $porcentajeDescuento,
				'montoAbono' => $montoAbono,
	...
	public function buscarDeudasInscripcion($representante = null, $anioInscripcion = null)
	{
		...
		$transacciones = $this->Studenttransactions->find('all')
        	...
        	->where(
				[
					'Studenttransactions.ano_escolar' => $anioInscripcion,
					'Studenttransactions.amount_dollar >' => 0,
					'Students.parentsandguardian_id' => $representante->id,
					'Students.student_condition' => 'Regular',
					'Students.balance' => $anioInscripcion
				]);
...
// Función para gestionar el servicio educativo de los estudiantes
	public function servicioEducativo($periodoEscolar = null, $idFamilia = null, $idEstudiante = null, $indicadorExoneracion = null)
	{
		...
		$matriculasEstudiantesEncontradas = '';
		$cantidadEstudiantes = 0;
		$indicadorBusquedaEstudiantes = 0;
		$indicadorActualizacionExoneracion = 0;

		$this->loadModel('Schools');
		$escuela = $this->Schools->get(2);
		$anioPeriodoActual = $escuela->current_school_year;

		$anioPeriodoAnterior = $escuela->current_school_year - 1;
		$anioPeriodoProximo = $escuela->current_school_year + 1;
		$anioPeriodoProximo2 = $escuela->current_school_year + 2;

		$periodoEscolarAnterior = $anioPeriodoAnterior . "-" . $anioPeriodoActual;
		...
		{
			$anioEscolarInscripcion = substr($periodoEscolar, 0, 4);
			$matricula_anio = 'Matrícula ' . $anioEscolarInscripcion;
			$matriculasEstudiantesEncontradas = $this->Studenttransactions->find('all')
				->contain(['Students'])
				->where([
					'Studenttransactions.transaction_description' => $matricula_anio,
					'Studenttransactions.amount_dollar >' => 0,
					'Students.id >' => 1,
					'Students.number_of_brothers' => $anioEscolarInscripcion,
					'Students.student_condition' => 'Regular'
				])
				->order(['Students.surname' => 'ASC', 'Students.second_surname' => 'ASC', 'Students.first_name' => 'ASC', 'Students.second_name' => 'ASC']);

			$cantidadEstudiantes = $matriculasEstudiantesEncontradas->count();

			foreach ($matriculasEstudiantesEncontradas as $matricula)
			{
				if ($matricula->student_id)
				{
					$anioEstudiante = $matricula->student->number_of_brothers;
					$criteriosTransaccionEstudiante[] = [
						'student_id' => $matricula->student_id,
						'transaction_description' => 'Servicio educativo ' . $anioEstudiante
					];
				}
			}
		}
		...
		{
			$indicadorBusquedaEstudiantes = 1;
			$estudiantesEncontrados = $this->Studenttransactions->Students->find('all')
				->where(['parentsandguardian_id' => $idFamilia, 'student_condition' => 'Regular'])
				->order(['Students.surname' => 'ASC', 'Students.second_surname' => 'ASC', 'Students.first_name' => 'ASC', 'Students.second_name' => 'ASC']);

			foreach ($estudiantesEncontrados as $estudiante)
			{
				$anioEstudiante = $estudiante->number_of_brothers;
				$criteriosTransaccionEstudiante[] = [
		...
		if (!empty($criteriosTransaccionEstudiante))
		{
			$condicionesOR = [];
			$criteriosProcesados = [];
			foreach ($criteriosTransaccionEstudiante as $criterio)
			{
				$clave = $criterio['student_id'] . '_' . $criterio['transaction_description'];
				if (!isset($criteriosProcesados[$clave]))
				{
					$condicionesOR[] = [
						'student_id' => $criterio['student_id'],
						'transaction_description' => $criterio['transaction_description']
					];
					$criteriosProcesados[$clave] = true;
				}
			}

			if (!empty($condicionesOR))
			{
				$consultaTransaccionesServicioEducativo = $this->Studenttransactions->find()
					->where([
						'OR' => $condicionesOR
					]);

				foreach ($consultaTransaccionesServicioEducativo as $transaccion)
				{
					$clave = $transaccion->student_id . '_' . $transaccion->transaction_description;
					$todasLasTransaccionesServicioEducativo[$clave] = $transaccion;
				}
			}
		}

		if (!empty($matriculasEstudiantesEncontradas))
		{
			foreach ($matriculasEstudiantesEncontradas as $matricula)
			{
				$anioEstudiante = $matricula->student->number_of_brothers;
				$descripcionTransaccion = 'Servicio educativo ' . $anioEstudiante;
				$clave = $matricula->student_id . '_' . $descripcionTransaccion;

				$transaccionServicioEducativo = $todasLasTransaccionesServicioEducativo[$clave] ?? null;

				$datosProcesados = null;
				// Llama a tu función procesarServicioEducativo de tu plugin
				if ($transaccionServicioEducativo)
				{
					// Adaptar la llamada a cómo está disponible en tu entorno
					$datosProcesados = $this->procesarServicioEducativo($transaccionServicioEducativo, $matricula->student);
				}

				$matricula->set('servicio_educativo', [
					'tarifaCuota' => $datosProcesados['tarifaCuota'] ?? null,
					'montoAbono' => $datosProcesados['montoAbono'] ?? null,
					'saldoCuota' => $datosProcesados['saldoCuota'] ?? null,
					'mensaje' => $datosProcesados['mensaje'] ?? 'Transacción no encontrada o error en proceso',
					'codigo_retorno' => $datosProcesados['codigo_retorno'] ?? 1
				]);
			}
		}
		...
		if (!empty($estudiantesEncontrados))
		{
			foreach ($estudiantesEncontrados as $estudiante)
			{
				$anioEstudiante = $estudiante->number_of_brothers;
				$descripcionTransaccion = 'Servicio educativo ' . $anioEstudiante;
				$clave = $estudiante->id . '_' . $descripcionTransaccion;
				...
				{
					// Adaptar la llamada a cómo está disponible en tu entorno
					$datosProcesados = $this->procesarServicioEducativo($transaccionServicioEducativo, $estudiante);
				}

				$estudiante->set('servicio_educativo', [
					'tarifaCuota' => $datosProcesados['tarifaCuota'] ?? null,
					'montoAbono' => $datosProcesados['montoAbono'] ?? null,
					'saldoCuota' => $datosProcesados['saldoCuota'] ?? null,
					'mensaje' => $datosProcesados['mensaje'] ?? 'Transacción no encontrada o error en proceso',
		...
		{
			$indicadorActualizacionExoneracion = 1;
			$estudiante = $this->Studenttransactions->Students->get($idEstudiante, [
				'contain' => []
			]);

			if ($indicadorExoneracion == 0)
			{
				$estudiante->servicio_educativo_exonerado = 0;
			}
			else
			{
				$estudiante->servicio_educativo_exonerado = 100;
			}
			...
			if ($this->Studenttransactions->Students->save($estudiante))
			{
				if ($indicadorExoneracion == 0)
				{
					$this->Flash->success(__('La exoneración del servicio educativo fue eliminada exitosamente'));
				}
				else
				{
					$this->Flash->success(__('El estudiante fue exonerado exitosamente'));
				}
			}
			else
			{
				if ($indicadorExoneracion == 0)
				{
					$this->Flash->error(__('No se pudo eliminar la exoneracion del servicio educativo, por favor intente nuevamente'));
				}
				else
				{
					$this->Flash->error(__('No se pudo exonerar el estudiante, por favor intente nuevamente'));
				}
			}
		}
