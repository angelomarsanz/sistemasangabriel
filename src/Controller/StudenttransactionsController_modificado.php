		...
		{ 
			...
			foreach ($mesesTarifas as $mesesTarifa)
			{
				if ($mesesTarifa["anoMes"] == $anio_mes_transaccion)
				{
					...
					{
						$monto_cuota = round(($mesesTarifa["tarifaDolar"] * (100 - $transaccion->student->discount)) / 100);
					}
					else
					{
						$monto_cuota = $mesesTarifa["tarifaDolar"];
					}
					break;
				}
			}
			...
			else
			{
				...
				if ($transaccion->amount_dollar < $cuota_menos_descuento_por_ajuste)
				{
