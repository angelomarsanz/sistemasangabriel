...
<?php if ($accountService == 0): ?>
	<?php if ($bill->fiscal == 1): ?>
		...
		<div style="font-size: 9px; line-height: 11px;">
			...
			<div style='text-align: center'>
				<h3>U.E. Colegio XXXXXXX</h3>
			</div>
			<div style='text-align: center'>
				<h5>Dirección xxxxx, teléfono xxxxx, Rif: J-nnnnn-nnnn</h5>
			</div>
			<br /><br>
				...
				<tbody>
					<tr>
						...
						<?php if ($bill->tipo_documento == "Nota de crédito"): ?>
							<td style="width:20%; text-align: right;"><b>Nota de crédito Nro. <?= $bill->bill_number ?> Nro. Control 00-<?= $bill->control_number ?></b></td>
						<?php elseif ($bill->tipo_documento == "Nota de débito"): ?>
							<td style="width:20%; text-align: right;"><b>Nota de débito Nro. <?= $bill->bill_number ?> Nro. Control 00-<?= $bill->control_number ?></b></td>
						<?php else: ?>
							<td style="width:20%; text-align: right;"><b>Factura Nro. <?= $bill->bill_number ?> Nro. Control 00-<?= $bill->control_number ?></b></td>
						<?php endif; ?>
					</tr>
					<tr>
					...
					<tr>
						...
						<?php if($numeroFacturaAfectada > 0): ?>
							<td style='width:20%; text-align: right;'> <?= "Factura afectada: Nro. " . $numeroFacturaAfectada . " Control 00-" . $controlFacturaAfectada ?></td>
						<?php else: ?>
		...
		<div style='text-align: center; font-size: 10px; line-height: 11px;'>
		...
		<div style="font-size: 9px; line-height: 11px;">
			...
			<table style="width:100%">
				<tbody>
					...
					<tr>
						<td style='width:80%;'>C.I./RIF: <?= $bill->identification ?></td>
						<td style="width:20%; text-align: right;">Fecha: <?= $bill->date_and_time->format('d-m-Y H:i:s').' a.m.' ?></td>
					</tr>
					<tr>
						<td style='width:80%;'>Teléfono: <?= $bill->tax_phone ?></td>
						<td style="width:20%; text-align: right;"><?= $bill->school_year ?></td>
	...
	<?php elseif ($indicadorAnticipo == 1): ?>
		...
		<div style="font-size: 9px; line-height: 11px;">
			<?php if ($bill->annulled == 1): ?>
				<h1 style="text-align:center;">Recibo de Anticipo Anulado <?= $bill->date_annulled->format('d-m-Y').'a.m.' ?></h1>
			<?php endif; ?>
			<table style="width:100%">
				<tbody>
					<tr>
