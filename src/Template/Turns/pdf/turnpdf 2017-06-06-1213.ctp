    ...
    <div>
        ...
        foreach ($paymentsTurn as $paymentsTurns): 
            ...
            <tr id=<?= $paymentsTurns->id ?>>
                <td><?= h($paymentsTurns->created->format('d-m-Y H:i:s')) ?></td>
                <td><?= h($paymentsTurns->bill_number) ?></td>
                <td><?= h($paymentsTurns->name_family) ?></td>
                <td><?= h(number_format($paymentsTurns->amount, 2, ",", ".")) ?></td>
                <td><?= h($paymentsTurns->bank) ?></td>
                <td><?= h($paymentsTurns->serial) ?></td>
            </tr>
        <?php 
        endforeach; ?>
                        </tbody>
