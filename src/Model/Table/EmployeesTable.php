...
class EmployeesTable extends Table
{
    ...
    public function findOnly(Query $query, array $options)
    {
        $query->contain(['Positions'])
            ->where([['Employees.responsible_user' => $options['responsible_user']],
            ['Employees.reason_withdrawal' => 'Activo']])
    ...
    public function findOnlyId(Query $query, array $options)
    {
        $query->contain(['Positions'])
            ->where(['Employees.id' => $options['id']])
            ->order(['Employees.created' => 'DESC']);
